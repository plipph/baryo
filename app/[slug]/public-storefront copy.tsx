...
export function PublicStorefront({
  ...
  return (
    <div
      className={`min-h-screen ${currentTheme.page}`}
    >
      {/* SEARCH */}
      <div
        className={`sticky top-[env(safe-area-inset-top)] z-20 mb-6 border p-3 md:mb-8 md:p-4 shadow-lg backdrop-blur-xl ${cardRadiusClass} ${currentTheme.card}`}
      >
        <div
          className={`flex items-center gap-3 px-4 py-4 ${buttonRadiusClass} ${currentTheme.surface}`}
        >
      ...
      {filteredItems.length === 0 && (
        <div className="rounded-[2rem] border border-dashed border-[#D8C7B4] bg-white/60 p-10 md:p-12 text-center">
          <h3
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 md:gap-6">
                  {items.map((item) => (
                    <button
                      ...
                      className={`group overflow-hidden border shadow-sm transition-all active:scale-[0.98] md:hover:-translate-y-1 md:hover:shadow-2xl text-left w-full ${cardRadiusClass} ${currentTheme.card}`}
                    >
                      {/* IMAGE */}
                      {item.image_url ? (
                      ...
                      <div className="p-5 md:p-6">
                        <div className="flex items-start justify-between gap-4">
                          <div className="min-w-0 flex-1">
                            ...
                            {item.description && (
                              <p
                                className={`mt-3 text-sm leading-relaxed ${currentTheme.muted}`}
                              >
        ...
        selectedItem && (
          ...
          >
            <div
              ...
              className={`relative w-full max-w-2xl max-h-[90vh] overflow-y-auto border shadow-2xl ${cardRadiusClass} ${currentTheme.card}`}
            >
              {/* CLOSE */}
              <button
                onClick={() => {
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    ...
                    {selectedItem.description && (
                      <p
                        className={`mt-4 text-base leading-relaxed ${currentTheme.muted}`}
                      >
                        {
                          selectedItem.description
                        }
                  ...
                  {selectedItem.price !==
                    null && (
                    <div
                      className={`${buttonRadiusClass} shrink-0 px-5 py-3 text-sm font-bold text-white`}
                      style={{
                        backgroundColor:
                          accentColor,
                      }}
                  ...
                  <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                    {links.map((link) => (
                      <a
                        ...
                        className={`inline-flex w-full sm:w-auto items-center justify-center gap-2 px-5 py-4 text-sm font-semibold text-white transition-all hover:opacity-90 ${buttonRadiusClass}`}
                        style={{
                          backgroundColor:
                            accentColor,
                        }}
