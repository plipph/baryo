...
Phase 2 adds profile-first authentication routing only. Registration now directs users to `/account`; login still enters through `/dashboard`, which preserves existing owner behavior and redirects users without a business to `/account`. `/account` and `/account/settings` are authenticated, non-editing account destinations. Account navigation exposes Profile, Settings, and Register Business for users without a business; owner navigation retains business management and adds Profile/Settings links. Owner-only catalog, appearance, and analytics routes redirect non-owners to `/account`; `/dashboard/business` remains the optional business-registration route.

**Known limitation:** Phase 1's profile-provisioning migration must be applied before production registration uses the trigger-backed Profile foundation. Profile/settings editing and centralized permission checks remain out of scope for Phase 2.

## Phase 3 Implementation Record

Phase 3 implements the authenticated Profile experience. `/profile` supports self-service display-name, city, province, and bio editing; email is read-only. `/settings` supports display-name editing and presents the current email. `/account` and `/account/settings` remain compatibility redirects to the new routes. Users without a business see the friendly business-registration/discovery onboarding card on their Profile page, while owners retain dashboard access and gain Profile/Settings navigation links.

The Phase 3 migration adds nullable Profile fields and self-only read/update RLS policies. Avatar display uses `avatar_url` when present and initials otherwise. Upload is intentionally deferred until a dedicated Supabase Storage bucket and policy are configured. Password change is not offered because the current Supabase authentication flow has no supported password-update implementation.
